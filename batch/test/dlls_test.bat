...
(
    for /f "usebackq tokens=1-4 delims=," %%a in ("pc_ip.csv") do (
        echo Poste %%a : %%b
        IF NOT EXIST "\\%%b\c$\Program Files (x86)\Cegid\Cegid Retail\CPOS Drivers\Nepting" (
            md "\\%%b\c$\Program Files (x86)\Cegid\Cegid Retail\CPOS Drivers\Nepting"
        )
        echo.
        echo --------- copy DLL
